...
async def chat_endpoint(request: Request):
    ...
    try:
        ...
        if session['name'] and session['phone'] and not session.get('telegram_sent', False):
            ...
            should_send = explicit_intent or session['product_mentioned'] or session.get('opening_height')
            
            if should_send:
                print(f"🚨 ОТПРАВЛЯЕМ ЗАЯВКУ В TELEGRAM!")
                full_conversation = "\n".join(session['text_parts'])
                
                # Сохраняем информацию о товаре в session для отправки
                if product_interest:
                    session['product_type'] = product_interest
                if session.get('opening_height'):
                    session['opening_params'] = f"Высота: {session['opening_height']}"
